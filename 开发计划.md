...
## 3. WebSocket 消息协议（JSON）(`cc.jfire.webcli.protocol`)
- 定义消息类型枚举和消息体（JSON 文本帧）
- JSON 框架使用 JfireBoot 自带的 Dson
...
### 5.1 删除 `src/main/java/cc/jfire/webcli/Application.java`

### 5.2 创建 `cc.jfire.webcli.WebApplication` 类
...
public class WebApplication {
    ...
    public void start() {
        ...
        HttpAppServer.StartParam param = new HttpAppServer.StartParam()
                .setChannelConfig(new ChannelConfig()
                        .setIp("127.0.0.1")
                        .setPort(port)
                        .setChannelGroup(ChannelConfig.DEFAULT_CHANNEL_GROUP))
                .setContext(context)
                .setWebDir("web")  // 静态资源目录，相对于 classpath
                .setWebSocketProcessor(wsHandler);

        server = HttpAppServer.start(param);
...
1. 修改 pom.xml 添加 pty4j 依赖
2. 创建 PTY 管理模块（PtyInstance、PtyManager）
3. 创建 WebSocket 消息协议类（MessageType、WsMessage、PtyInfo）
4. 创建 WebSocket 处理器（WebSocketHandler）
5. 删除 Application 启动类并创建 WebApplication
6. 创建前端页面（index.html）
...
7. `cc.jfire.webcli.WebApplication` - 应用启动类

## 删除的类
1. `cc.jfire.webcli.Application` - 旧启动类（Hello World）
...
> **关于 WebSocket 支持**：JfireBoot 已内置完整的 WebSocket 支持，包括：
> - `WebSocketUpgradeDecoder` - 自动处理 WebSocket 握手
> - `WebSocketFrameDecoder` - 自动解码 WebSocket 帧
> - `WebSocketFrameEncoder` - 自动编码 WebSocket 帧
>
> 因此不需要自己实现 `WebSocketUpgradeProcessor`，只需实现业务处理器 `WebSocketHandler` 即可。
